...
class TargetEnvWrapper(envs.Wrapper):
    ...
    def __init__(self, env, generator=None, generator_params=None, noise_dim=10):
    ...
    def reset(self, rng):
        ...
        if self.generator is not None and self.generator_params is not None:
            if is_batched:
...
def train(
    ...
    def update_step_context_gen_disc(carry, trajectories):
        training_state, key = carry
    ...
    def train_steps(training_state, buffer_state, key):
        ...
        (training_state, _), metrics2 = jax.lax.scan(update_step_context_gen_disc, (training_state, training_key), trajectories)
