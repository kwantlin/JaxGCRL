...
for seed in 1 ; do
  XLA_PYTHON_CLIENT_MEM_FRACTION=.95 MUJOCO_GL=egl CUDA_VISIBLE_DEVICES=0 python main.py "$method" \
    --wandb_project_name sudoku-bmm --wandb_group first_run --exp_name ${env}_${seed} --num_evals 50 \
    --seed ${seed} --total_env_steps 20000000 --batch_size 256 --num_envs 512 \
    ...
    --train_step_multiplier 4 --repetition_factor 2 --log_wandb
  done
