...
submit_job() {
    ...
    local slurm_script="temp_${env}_hilp_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.slurm"

    # Create a temporary SLURM script for this job
    cat > ${slurm_script} << EOF
#!/bin/bash

#SBATCH --job-name=${env}_hilp_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
#SBATCH --output=logs/della_${env}_hilp_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.out
#SBATCH --error=logs/della_${env}_hilp_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.err

eval "\$(conda shell.bash hook)"
conda activate jaxgcrl


python training_hilp.py \
    --project_name test --group_name first_run --exp_name ${env}-hilp-della_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier} --num_evals ${num_evals} \
...
env=ant
submit_job $env 1 30000000 512 1024 100 1 "icml"
...
env=simple_u_maze
submit_job $env 1 40000000 1024 256 50 1 "icml"
# submit_job $env 2 40000000 1024 256 50 1 "rebuttal"
# submit_job $env 3 40000000 1024 256 50 1 "rebuttal"
# submit_job $env 4 40000000 1024 256 50 1 "rebuttal"
# submit_job $env 5 40000000 1024 256 50 1 "rebuttal"

env=reacher
submit_job $env 1 30000000 1024 256 100 1 "icml"
...
env=pusher_easy
submit_job $env 1 60000000 256 512 100 1 "icml"
...
env=ant_posvel
submit_job $env 1 120000000 256 512 500 1 "icml"
...
env=ant_angvel
submit_job $env 1 120000000 256 512 50 1 "icml"
