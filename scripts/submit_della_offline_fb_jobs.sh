...
python training_offline_fb.py \
    --project_name test --group_name first_run --exp_name offline-${env}-fb-offline-della_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier} --num_evals ${num_evals} \
...
DATASET_DIR="/scratch/gpfs/EYSENBACH/kw2960/JaxGCRL/data"

env=ant
dataset_path="${DATASET_DIR}/${env}_offline_crl_dataset.npz"
submit_job $env 1 30000000 512 1024 100 1 "rebuttal" "${dataset_path}"
...
env=reacher
dataset_path="${DATASET_DIR}/${env}_offline_crl_dataset.npz"
submit_job $env 1 30000000 1024 256 100 1 "rebuttal" "${dataset_path}"

env=pusher_easy
dataset_path="${DATASET_DIR}/${env}_offline_crl_dataset.npz"
submit_job $env 1 60000000 256 512 100 1 "rebuttal" "${dataset_path}"
...
# env=ant_angvel
# dataset_path="${DATASET_DIR}/${env}_offline_crl_dataset.npz"
# submit_job $env 1 120000000 256 512 50 1 "rebuttal" "${dataset_path}"
