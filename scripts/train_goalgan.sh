...
for seed in 1 ; do
  XLA_PYTHON_CLIENT_MEM_FRACTION=.95 MUJOCO_GL=egl CUDA_VISIBLE_DEVICES=0 python training_goalgan.py \
    --project_name test --group_name first_run --exp_name ${env}-goalgan --num_evals 50 \
    ...
    --contrastive_loss_fn fb --energy_fn l2 \
