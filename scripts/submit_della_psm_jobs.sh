...
submit_job() {
    ...
    local slurm_script="temp_${env}_psm_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.slurm"

    # Create a temporary SLURM script for this job
    cat > ${slurm_script} << EOF
#!/bin/bash

#SBATCH --job-name=${env}_psm_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
#SBATCH --output=logs/della_${env}_psm_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.out
#SBATCH --error=logs/della_${env}_psm_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier}.err

eval "\$(conda shell.bash hook)"
conda activate jaxgcrl


python training_psm.py \
    --project_name test --group_name first_run --exp_name ${env}-psm-della_${seed}_${sanitized_notes}_${num_timesteps}_${batch_size}_${num_envs}_${num_evals}_${train_step_multiplier} --num_evals ${num_evals} \
...
env=ant
submit_job $env 1 30000000 512 1024 100 1 "rebuttal-redo"
...
env=simple_u_maze
submit_job $env 1 40000000 1024 256 50 1 "rebuttal-redo"
...
env=reacher
submit_job $env 1 30000000 1024 256 100 1 "rebuttal-redo"
...
env=pusher_easy
submit_job $env 1 60000000 256 512 100 1 "rebuttal-redo"
...
env=ant_posvel
submit_job $env 1 120000000 256 512 500 1 "rebuttal-redo"
...
env=ant_angvel
submit_job $env 1 120000000 256 512 50 1 "rebuttal-redo"
