...
for i, (env_name, data) in enumerate(zip(environments, data_list)):
    ...
    ax.set_ylabel('Imitation Score (%)' if i == 0 else '', fontsize=12)
    ax.set_xlabel('')
...
# Define colors for consistency
traj_colors = {'CRL': '#1f77b4', 'CIRL': '#ff7f0e', 'BC': '#d62728'}
...
for i, (env_name, data) in enumerate(zip(environments, data_list)):
    ...
    if i == 0:
        handles, labels = ax.get_legend_handles_labels()
        legend1 = ax.legend(handles=handles, labels=labels, loc='upper left', title='Method Type')
...
# Define colors for consistency
pretrain_colors = {'BC': '#d62728', 'NN': 'purple', 'FB': '#2ca02c', 'CRL': '#1f77b4', 'CIRL': '#ff7f0e'}
...
# Define colors for consistency
oracle_colors = {'CRL + Oracle Sampling': '#1f77b4', 'CRL + GoalKDE (CIRL)': '#ff7f0e'}
