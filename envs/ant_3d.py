...
class Ant3D(PipelineEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # Keep target stationary by manually correcting its state components.
        # This is more efficient than re-running forward kinematics.
        q = pipeline_state.q.at[-3:].set(pipeline_state0.q[-3:])
        qd = pipeline_state.qd.at[-3:].set(0)
        x_pos = pipeline_state.x.pos.at[-1].set(pipeline_state0.x.pos[-1])
        x_rot = pipeline_state.x.rot.at[-1].set(pipeline_state0.x.rot[-1])
        pipeline_state = pipeline_state.replace(
            q=q, qd=qd, x=pipeline_state.x.replace(pos=x_pos, rot=x_rot)
        )
