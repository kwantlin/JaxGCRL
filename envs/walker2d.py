...
class Walker2d(PipelineEnv):
  ...
  def __init__(
    ...
    """
    path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "assets", "walker2d.xml")
    sys = mjcf.load(path)
...
class WalkerForward(Walker2d):
  """A walker that is rewarded for moving forward at a high velocity."""
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    forward_reward = self._forward_reward_weight * jp.where(
        x_velocity > self._min_forward_velocity, x_velocity, 0.0
    )
...
class WalkerJump(Walker2d):
  """A walker that is rewarded for jumping."""
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    obs = self._get_obs(pipeline_state)
    reward = jump_reward + healthy_reward - ctrl_cost
    done = 1.0 - is_healthy if self._terminate_when_unhealthy else 0.0

    # Calculate x_velocity for logging
    x_velocity = (
        pipeline_state.x.pos[0, 0] - pipeline_state0.x.pos[0, 0]
    ) / self.dt
